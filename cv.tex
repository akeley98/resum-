...
\item Author Vulkan samples for
  \texttt{VK\_EXT\_graphics\_pipeline\_library},
  \texttt{VK\_KHR\_timeline\_semaphore},\\
  \texttt{GL\_KHR\_shader\_subgroup\_shuffle},
  \texttt{VK\_NV\_inherited\_viewport\_scissor}, and ray tracing extensions.
...
\item Consulting with business partners on integrating NVIDIA
  technology: DMM, \texttt{GL\_NV\_path\_rendering}.
\end{enumerate}
\filbreak
...
only 12 triangles to draw.} for each chunk's visible voxels, and draws
the AABBs themselves with a raycasting fragment shader that only
